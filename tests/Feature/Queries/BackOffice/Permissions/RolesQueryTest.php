<?php

namespace Tests\Feature\Queries\BackOffice\Permissions;

use App\GraphQL\Queries\BackOffice;
use App\GraphQL\Queries\BackOffice\Admins\AdminsQuery;
use App\GraphQL\Queries\BackOffice\Permissions\RolesQuery;
use App\Modules\Admin\Models\Admin;
use App\Modules\Permissions\Enums\Guard;
use App\Modules\Permissions\Models\Role;
use App\Permissions\Admins\AdminListPermission;
use App\Permissions\Roles\RoleListPermission;
use Carbon\CarbonImmutable;
use Illuminate\Foundation\Testing\DatabaseTransactions;
use Tests\Builders\Admins\AdminBuilder;
use Tests\Builders\Permissions\PermissionBuilder;
use Tests\Builders\Permissions\RoleBuilder;
use Tests\TestCase;

class RolesQueryTest extends TestCase
{
    use DatabaseTransactions;

    public const QUERY = RolesQuery::NAME;

    protected AdminBuilder $adminBuilder;
    protected PermissionBuilder $permissionBuilder;
    protected RoleBuilder $roleBuilder;

    public function setUp(): void
    {
        parent::setUp();
        $this->adminBuilder = resolve(AdminBuilder::class);
        $this->permissionBuilder = resolve(PermissionBuilder::class);
        $this->roleBuilder = resolve(RoleBuilder::class);

        $this->langInit();
    }

    /** @test */
    public function success_paginator(): void
    {
        $this->loginAsSuperAdmin();

        $m_1 = $this->roleBuilder->create();
        $m_2 = $this->roleBuilder->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStr()
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $m_2->id],
                            ['id' => $m_1->id],
                        ],
                        'meta' => [
                            'total' => 2
                        ],
                    ]
                ]
            ])
        ;
    }

    /** @test */
    public function success_paginator_as_admin(): void
    {
        $perm = $this->permissionBuilder
            ->guard(Guard::ADMIN())
            ->name(RoleListPermission::KEY)->create();
        $model = $this->loginAsAdmin(null, $perm);

        $m_1 = $this->roleBuilder->create();
        $m_2 = $this->roleBuilder->create();
        $m_3 = $this->roleBuilder->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStr()
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $m_3->id],
                            ['id' => $m_2->id],
                            ['id' => $m_1->id],
                            ['id' => $model->role->id],
                        ],
                        'meta' => [
                            'total' => 4
                        ],
                    ]
                ]
            ])
        ;
    }

    /** @test */
    public function success_with_page(): void
    {
        $this->loginAsSuperAdmin();

        $this->roleBuilder->create();
        $this->roleBuilder->create();
        $this->roleBuilder->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrWithPage(2)
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'meta' => [
                            'total' => 3,
                            'per_page' => 10,
                            'current_page' => 2,
                            'from' => null,
                            'to' => null,
                            'last_page' => 1,
                            'has_more_pages' => false,
                        ],
                    ]
                ]
            ])
        ;
    }

    protected function getQueryStrWithPage($page): string
    {
        return sprintf(
            '
            {
                %s (page: %s) {
                    data {
                        id
                    }
                    meta {
                        total
                        per_page
                        current_page
                        from
                        to
                        last_page
                        has_more_pages
                    }
                }
            }',
            self::QUERY,
            $page
        );
    }

    /** @test */
    public function success_with_per_page(): void
    {
        $this->loginAsSuperAdmin();

        $this->roleBuilder->create();
        $this->roleBuilder->create();
        $this->roleBuilder->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrWithPerPage(2)
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'meta' => [
                            'total' => 3,
                            'per_page' => 2,
                            'current_page' => 1,
                            'from' => 1,
                            'to' => 2,
                            'last_page' => 2,
                            'has_more_pages' => true,
                        ],
                    ]
                ]
            ])
        ;
    }

    protected function getQueryStrWithPerPage($perPage): string
    {
        return sprintf(
            '
            {
                %s (per_page: %s) {
                    data {
                        id
                    }
                    meta {
                        total
                        per_page
                        current_page
                        from
                        to
                        last_page
                        has_more_pages
                    }
                }
            }',
            self::QUERY,
            $perPage
        );
    }

    /** @test */
    public function success_empty(): void
    {
        $this->loginAsSuperAdmin();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStr()
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'meta' => [
                            'total' => 0
                        ],
                    ]
                ]
            ])
        ;
    }

    protected function getQueryStr(): string
    {
        return sprintf(
            '
            {
                %s {
                    data {
                        id
                    }
                    meta {
                        total
                        per_page
                        current_page
                        from
                        to
                        last_page
                        has_more_pages
                    }
                }
            }',
            self::QUERY
        );
    }

    /** @test */
    public function filter_by_id(): void
    {
        $this->loginAsSuperAdmin();

        $perm_1 = $this->permissionBuilder->create();
        $perm_2 = $this->permissionBuilder->create();

        /** @var $model Role */
        $model = $this->roleBuilder
            ->withTranslation()
            ->permissions($perm_1, $perm_2)
            ->create();
        $this->roleBuilder->create();
        $this->roleBuilder->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrById($model->id)
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            [
                                'id' => $model->id,
                                'name' => $model->name,
                                'guard' => $model->guard_name,
                                'translation' => [
                                    'title' => $model->translation->title
                                ],
                            ]
                        ],
                        'meta' => [
                            'total' => 1
                        ],
                    ]
                ]
            ])
            ->assertJsonCount(2, 'data.'.self::QUERY.'.data.0.permissions')
        ;
    }

    protected function getQueryStrById($id): string
    {
        return sprintf(
            '
            {
                %s (id: %s){
                    data {
                        id
                        name
                        guard
                        translation {
                            title
                        }
                        permissions {
                            name
                        }
                    }
                    meta {
                        total
                    }
                }
            }',
            self::QUERY,
            $id
        );
    }

    /** @test */
    public function filter_by_ids(): void
    {
        $this->loginAsSuperAdmin();

        /** @var $model_1 Role */
        $model_1 = $this->roleBuilder->create();
        $model_2 = $this->roleBuilder->create();
        $this->roleBuilder->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrByIds([$model_1->id, $model_2->id])
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_2->id],
                            ['id' => $model_1->id],
                        ],
                        'meta' => [
                            'total' => 2
                        ],
                    ]
                ]
            ])
        ;
    }

    protected function getQueryStrByIds(array $ids): string
    {
        return sprintf(
            '
            {
                %s (ids: [%s, %s]){
                    data {
                        id
                    }
                    meta {
                        total
                    }
                }
            }',
            self::QUERY,
            $ids[0],
            $ids[1],
        );
    }

    /** @test */
    public function search_by_name(): void
    {
        $this->loginAsSuperAdmin();

        /** @var $model_1 Role */
        $model_1 = $this->roleBuilder->name("admin")->create();
        $model_2 = $this->roleBuilder->name("admin writer")->create();
        $this->roleBuilder->name("manager")->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrByName("admin")
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_2->id],
                            ['id' => $model_1->id],
                        ],
                        'meta' => [
                            'total' => 2
                        ],
                    ]
                ]
            ])
        ;
    }

    protected function getQueryStrByName(string $value): string
    {
        return sprintf(
            '
            {
                %s (name: "%s"){
                    data {
                        id
                    }
                    meta {
                        total
                    }
                }
            }',
            self::QUERY,
            $value,
        );
    }

    /** @test */
    public function filter_by_guard(): void
    {
        $this->loginAsSuperAdmin();

        /** @var $model_1 Admin */
        $model_1 = $this->roleBuilder->guard("user_guard")->create();
        $model_2 = $this->roleBuilder->guard("admin_guard")->create();
        $this->roleBuilder->name("admin_guard")->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrByGuard("user_guard")
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_1->id],
                        ],
                        'meta' => [
                            'total' => 1
                        ],
                    ]
                ]
            ])
        ;
    }

    protected function getQueryStrByGuard(string $value): string
    {
        return sprintf(
            '
            {
                %s (guard: %s){
                    data {
                        id
                    }
                    meta {
                        total
                    }
                }
            }',
            self::QUERY,
            $value,
        );
    }

    /** @test */
    public function search_by_title(): void
    {
        $this->loginAsSuperAdmin();

        $model_1 = $this->roleBuilder->withTranslation(['en' => ['title' => "admin writer"]])->create();
        $model_2 = $this->roleBuilder->withTranslation(['en' => ['title' => "admin"]])->create();
        $this->roleBuilder->withTranslation(['en' => ['title' => "writer"]])->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySearchTitle('admin')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_2->id],
                            ['id' => $model_1->id],
                        ],
                        'meta' => [
                            'total' => 2
                        ],
                    ]
                ]
            ])
        ;
    }

    protected function getQueryStrBySearchTitle(string $value): string
    {
        return sprintf(
            '
            {
                %s (title: "%s"){
                    data {
                        id
                    }
                    meta {
                        total
                    }
                }
            }',
            self::QUERY,
            $value,
        );
    }

    /** @test */
    public function sort_by_id(): void
    {
        $this->loginAsSuperAdmin();

        /** @var $model_1 Role */
        $model_1 = $this->roleBuilder->create();
        $model_2 = $this->roleBuilder->create();
        $model_3 = $this->roleBuilder->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySort('id-desc')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_3->id],
                            ['id' => $model_2->id],
                            ['id' => $model_1->id],
                        ],
                        'meta' => [
                            'total' => 3
                        ],
                    ]
                ]
            ])
        ;

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySort('id-asc')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_1->id],
                            ['id' => $model_2->id],
                            ['id' => $model_3->id],
                        ],
                        'meta' => [
                            'total' => 3
                        ],
                    ]
                ]
            ])
        ;
    }

    /** @test */
    public function sort_by_name(): void
    {
        $this->loginAsSuperAdmin();

        /** @var $model_1 Admin */
        $model_1 = $this->roleBuilder->setData(['name' => 'ben'])->create();
        $model_2 = $this->roleBuilder->setData(['name' => 'allan'])->create();
        $model_3 = $this->roleBuilder->setData(['name' => 'zet'])->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySort('name-asc')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_2->id],
                            ['id' => $model_1->id],
                            ['id' => $model_3->id],
                        ],
                        'meta' => [
                            'total' => 3
                        ],
                    ]
                ]
            ])
        ;

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySort('name-desc')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_3->id],
                            ['id' => $model_1->id],
                            ['id' => $model_2->id],
                        ],
                        'meta' => [
                            'total' => 3
                        ],
                    ]
                ]
            ])
        ;
    }

    /** @test */
    public function sort_by_created(): void
    {
        $this->loginAsSuperAdmin();

        $data = CarbonImmutable::now()->subMonth();

        /** @var $model_1 Role */
        $model_1 = $this->roleBuilder->setData(['created_at' => $data->addDays(10)])->create();
        $model_2 = $this->roleBuilder->setData(['created_at' => $data->addDays(4)])->create();
        $model_3 = $this->roleBuilder->setData(['created_at' => $data->addDays(2)])->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySort('created_at-asc')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_3->id],
                            ['id' => $model_2->id],
                            ['id' => $model_1->id],
                        ],
                        'meta' => [
                            'total' => 3
                        ],
                    ]
                ]
            ])
        ;

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySort('created_at-desc')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_1->id],
                            ['id' => $model_2->id],
                            ['id' => $model_3->id],
                        ],
                        'meta' => [
                            'total' => 3
                        ],
                    ]
                ]
            ])
        ;
    }


    public function sort_by_title(): void
    {
        $this->loginAsSuperAdmin();

        $data = CarbonImmutable::now()->subMonth();

        /** @var $model_1 Role */
        $model_1 = $this->roleBuilder->withTranslation(['en' => ['title' => "admin"]])->create();
        $model_2 = $this->roleBuilder->withTranslation(['en' => ['title' => "writer"]])->create();
        $model_3 = $this->roleBuilder->withTranslation(['en' => ['title' => "manager"]])->create();

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySort('title-asc')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_1->id],
                            ['id' => $model_3->id],
                            ['id' => $model_2->id],
                        ],
                        'meta' => [
                            'total' => 3
                        ],
                    ]
                ]
            ])
        ;

        $this->postGraphQLBackOffice([
            'query' => $this->getQueryStrBySort('title-desc')
        ])
            ->assertJson([
                'data' => [
                    self::QUERY => [
                        'data' => [
                            ['id' => $model_2->id],
                            ['id' => $model_3->id],
                            ['id' => $model_1->id],
                        ],
                        'meta' => [
                            'total' => 3
                        ],
                    ]
                ]
            ])
        ;
    }

    protected function getQueryStrBySort(string $value): string
    {
        return sprintf(
            '
            {
                %s (sort: "%s"){
                    data {
                        id
                    }
                    meta {
                        total
                    }
                }
            }',
            self::QUERY,
            $value,
        );
    }

    /** @test */
    public function not_perm(): void
    {
        $this->loginAsAdmin();

        $res = $this->postGraphQLBackOffice([
            'query' => $this->getQueryStr()
        ])
        ;

        $this->assertPermission($res);
    }

    /** @test */
    public function not_auth(): void
    {
        $res = $this->postGraphQLBackOffice([
            'query' => $this->getQueryStr()
        ])
        ;

        $this->assertUnauthorized($res);
    }
}

